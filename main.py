...
if filter_one == "":
    filter_one = False
elif filter_one in ["不要", "否", "False", "N", "F"]:
    filter_one = False
else:
...
if already_score == "":
    ...
    correct_df = pd.read_excel(far_associate_correct_ans)
    ...
    for _, row in df.iterrows():
        if row["Quiz Class"] in [1,2,3]:
            ...
            if len(row.Ans) == 1 and highest_score < 0.5:
                is_one_invalid_ans.append(True)
            else:
    ...
    df["is_one_invalid_ans"] = is_one_invalid_ans
